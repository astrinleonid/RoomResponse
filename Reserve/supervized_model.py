...
def build_supervised_model(dataset_path='mfcc_dataset.csv'):
    ...
    if 'Random Forest' in best_models:
        ...
        feature_importance_analysis(rf_model, feature_cols, X_test, y_test)

    return best_models, X_test, y_test, feature_cols
...
def feature_importance_analysis(rf_model, feature_cols, X_test, y_test):
    ...
    plt.figure(figsize=(12, 6))
    plt.plot(range(1, len(importances) + 1), np.cumsum(importances[indices]), 'o-')
...
if __name__ == "__main__":
    best_models, X_test, y_test, feature_cols = build_supervised_model('mfcc_dataset.csv')
